...
class MensagemModal(ctk.CTkToplevel):
...
class DialogoSelecionarArquivo(ctk.CTkToplevel):
...
class VisualizarImagem(ctk.CTkFrame):
    ...
    def _aplicar_filtro(self):
        if self.controlador.imagem_atual_pil is None:
            MensagemModal(self.winfo_toplevel(), "Aviso", "Carregue uma imagem antes de aplicar filtros.", tipo="aviso")
            return
    ...
    def _carregar_imagem(self):
        ...
        if not texto:
            MensagemModal(self.winfo_toplevel(), "Aviso", "Digite um caminho ou uma URL para carregar uma imagem.", tipo="aviso")
            return
        ...
        try:
            ...
            MensagemModal(self.winfo_toplevel(), "Erro", str(erro), tipo="erro")
...
class FiltrarImagem(ctk.CTkFrame):
    ...
    def _criar_widgets(self):
        ...
        for classe_filtro, nome_filtro in FILTROS_DISPONIVEIS:
            ctk.CTkButton(
                sidebar,
                text=nome_filtro,
                width=180,
    ...
    def _mostrar_original(self):
        if self.imagem_original is None:
            MensagemModal(self.winfo_toplevel(), "Aviso", "Carregue uma imagem antes de visualizar o original.", tipo="aviso")
            return
    ...
    def _baixar_imagem(self):
        if self.imagem_resultado is None:
            MensagemModal(self.winfo_toplevel(), "Aviso", "Nenhuma imagem filtrada para baixar.", tipo="aviso")
            return

        caminho = filedialog.asksaveasfilename(
            title="Salvar imagem",
            initialdir=self.controlador.imagem_atual_caminho or os.getcwd(),
            initialfile="imagem_filtrada.png",
            defaultextension=".png",
            filetypes=[("PNG", "*.png"), ("JPEG", "*.jpg *.jpeg"), ("Todos os arquivos", "*.*")],
        )
        if not caminho:
            return
        ...
        try:
            ...
            MensagemModal(self.winfo_toplevel(), "Sucesso", f"Imagem salva em:\n{caminho}", tipo="info")
        except Exception as erro:
            MensagemModal(self.winfo_toplevel(), "Erro", str(erro), tipo="erro")
